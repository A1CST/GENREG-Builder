...
LOG = open(os.path.join(HERE, "conversation_v4_results.txt"), "w")
...
if __name__ == "__main__":
    ...
    def gen_c(topic, maxlen=11, beta=7.0):
    ...
    def express(B, nc=90):
        ...
        cands = [g for g in (gen_c(topic) for _ in range(nc)) if len(g) >= 3]
        scored = [(margin(g, B), fluency(g), g) for g in cands]
        scored = [s for s in scored if s[0] > 0 and s[1] > -1]
        return max(scored, key=lambda s: s[0] + 0.15 * s[1])[2] if scored else topic[:3]
