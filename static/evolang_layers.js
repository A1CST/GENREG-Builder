...
(() => {
  ...
  const NODES = [
    ...
    { id: "sem_coh_local", name: "Sentence coherence", layer: "semantic", stages: ["fill"], status: "planned",
      group: "coherence", groupLabel: "Coherence (composed)",
      short: "fits the current sentence?",
      desc: "PLANNED, not yet attempted. Split out of a single \"topical drift\" idea, which conflated two different timescales into one compound question — see Theme consistency. This half: does the candidate fit a TIGHT, fast-moving centroid of the current sentence's content words." },
    { id: "sem_coh_theme", name: "Theme consistency", layer: "semantic", stages: ["fill"], status: "planned",
      group: "coherence", groupLabel: "Coherence (composed)",
      short: "fits the passage's broader theme?",
      desc: "PLANNED, not yet attempted. The passage-level half of the split-out \"topical drift\" idea — does the candidate fit a SLOW-moving centroid over the whole passage. Also absorbs what a separate \"Domain purity\" genome would have measured (categorical topic-domain membership) — kept as one genome, not two, since they were answering nearly the same question from different angles." },
    ...
    { id: "sem_parallel", name: "List parallelism", layer: "semantic", stages: ["fill"], status: "planned",
      short: "coordinated items, same type?",
      desc: "PLANNED, not yet attempted. When generating a coordinated list (\"X, Y, and Z\"), are the items distributionally the same kind of thing — nearest-neighbor similarity among coordinated slots." },
    ...
    { id: "sent_type", name: "Sentence type", layer: "structural", stages: ["skeleton"], status: "experimental",
      ...
      desc: "WIRED, real generation effect measured (2026-07-08). Probe: all 18 hand-picked question-openers (do/will/what/is...) scored above all 11 statement-openers. Wired: flips a coin at the corpus question-rate (8.19%) per sentence, biases the opener toward this genome's scores, forces '?'. Generation check: flagged-question rate landed at 8.2% (matches corpus almost exactly), ~50% of flagged sentences opened with a recognized question word (was ~0% before). `sent_type` toggle, OFF by default — no formal adj-hit/distinct guardrail sweep run yet, only this spot-check." },
    { id: "sent_lenplan", name: "Sentence length plan", layer: "structural", stages: ["skeleton"], status: "experimental",
      ...
      desc: "WIRED, but NO measured generation effect (2026-07-08). Probe passed weakly: mean long-sentence-opener score beat mean short-opener score (+0.53 vs -0.13, val_acc 0.605), but several openers tied on identical scores — the shared function-feature space is too coarse for a sharp cut. Wired the same shape as Sentence type (opener bias + boundary-probability reshaping toward/away from the 14-word median). Generation spot-check (20 samples x 260 words): median/mean/stdev sentence length came back essentially IDENTICAL on vs off — technically wired, practically inert at safe gammas. `lenplan` toggle, OFF by default." },
    ...
    { id: "definite", name: "Definiteness", layer: "semantic", stages: ["fill"], status: "planned",
      short: "\"a\" vs \"the\" — new or known entity?",
      desc: "PLANNED, deliberately NOT attempted yet (2026-07-08). \"a\" vs \"the\" tracks whether a noun's referent is a first mention or already known — nothing currently tracks entity identity across a passage to inform this choice. Flagged risk before spending a training run here: this is a per-CONTEXT decision, not a per-word lexical property — Clause count just cut on exactly that failure mode (word-alone signal was Spearman ~0 for a downstream/contextual question). Needs a real per-context feature (e.g. \"has this noun's word-id appeared in the last K words\") before this is worth training, not a plain unary opener-style genome." },
    { id: "transitivity", name: "Transitivity", layer: "structural", stages: ["fill"], status: "planned",
      short: "verb needs an object?",
      desc: "PLANNED, deliberately NOT attempted yet (2026-07-08). If a verb requires an object, one must follow. Word-level (not class-level), so it may dodge the redundancy wall that cut the earlier class-level Verb argument genome — but there are no POS tags anywhere in this pipeline, so \"is this word a verb, and does IT specifically require an object\" still needs a real mining/feature idea before a training run is worth it, same caution as Definiteness above." },

    { id: "whole_sent", name: "Whole-sentence scorer", layer: "abstraction", stages: ["revision"], status: "planned",
      group: "revision", groupLabel: "Revision (new stage — composed)",
      short: "read + score the full sentence",
      desc: "PLANNED, not yet attempted — REQUIRES a new pipeline stage (Revision) that doesn't exist yet. Every genome today scores locally, word by word, during generation. A post-hoc pass reading the complete sentence could reject or resample bad ones — inherently a composed judgment over other genomes' signals, hence abstraction-tier." },
    { id: "best_of_n", name: "Best-of-N", layer: "abstraction", stages: ["revision"], status: "planned",
      group: "revision", groupLabel: "Revision (new stage — composed)",
      short: "generate N, keep the best",
      desc: "PLANNED, not yet attempted — REQUIRES the new Revision stage. Generate N candidate sentences with the existing pipeline, unchanged, then pick the best via the Whole-sentence scorer. No new training — selection at the sentence level instead of the word level." },

    { id: "pronominal", name: "Pronominalization", layer: "semantic", stages: ["passage"], status: "planned",
      short: "\"the king\" then \"he\" after",
      desc: "PLANNED, not yet attempted — REQUIRES a new pipeline stage (Passage, cross-sentence). \"The king\" on first mention, \"he\" afterward — nothing currently tracks entity introduction vs. reference across sentence boundaries." },
    { id: "discourse_rel", name: "Discourse relation", layer: "abstraction", stages: ["passage"], status: "planned",
      ...
      desc: "PLANNED, not yet attempted — REQUIRES the new Passage stage. Does this sentence elaborate on, contrast with, or follow causally from the previous one? The evolved version of the already-DEFERRED Discourse connector genome (that one was a flat rule, not a real conditional predictor) — needs an actual semantic comparison between two sentences' content, hence abstraction-tier, not structural." },
    { id: "info_status", name: "Information status", layer: "semantic", stages: ["passage"], status: "planned",
      short: "given info first, new info last?",
      desc: "PLANNED, not yet attempted — REQUIRES the new Passage stage. English information structure: given (already-established) information tends to come before new information within a sentence. Needs cross-sentence tracking of what's already been established, so semantic rather than purely structural." },
  ];
  ...
  const STAGES = [
    ...
    { id: "revision", label: "Revision", sub: "NOT BUILT — whole-sentence", future: true },
    { id: "passage", label: "Passage", sub: "NOT BUILT — cross-sentence", future: true },
  ];
  const LAYERS = [
