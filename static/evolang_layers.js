...
(() => {
  ...
  const NODES = [
    ...
    { id: "whole_sent", name: "Whole-sentence scorer", layer: "abstraction", stages: ["revision"], status: "experimental",
      ...
      desc: "BUILT (2026-07-08) — the Revision stage now exists. `Service._sentence_score()` in `wordpipe_service.py` reads a complete sentence post-hoc and composes a fitness from already-evolved champions (semantic adjacency, opener/closer fit, alternation-violation count, no-repeat-violation count, a degenerate-length guard). No new training. Verified mechanically: correctly rank-orders 8 candidates from cleanest to most degenerate. Known bias: roughly additive over word count, so it favors SHORTER sentences — not yet corrected for length. Not yet exposed through the /evolang UI/API." },
    { id: "best_of_n", name: "Best-of-N", layer: "abstraction", stages: ["revision"], status: "experimental",
      group: "revision", groupLabel: "Revision (new stage — composed)",
      short: "generate N, keep the best",
      desc: "BUILT (2026-07-08). `Service.generate_revision(en, n_sentences, n_candidates, seed)` generates several candidate sentences per slot with the UNCHANGED existing pipeline (different seeds), scores each with the Whole-sentence scorer, keeps the best. No new training — selection at the sentence level instead of the word level. Not yet exposed through the /evolang UI/API — callable directly, no route/toggle wired up yet." },

    { id: "pronominal", name: "Pronominalization", layer: "semantic", stages: ["passage"], status: "experimental",
      short: "\"the king\" then \"he\" after",
      desc: "WIRED, real generation effect measured (2026-07-08) — the first real Passage-stage genome, and it needed NO new training: reuses the No-repeat genome's `recent` buffer as the entity-recency signal. A content word re-mentioned within the last 15 words is replaced by a generic pronoun (\"it\", no gender/number modeling) 60% of the time. Generation check: 'it' frequency rose 0.77%→1.11% of words (+44% relative) with the toggle on — a real, measurable change. `pronominal` toggle, OFF by default." },
