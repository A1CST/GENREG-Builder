...
(() => {
  ...
  const NODES = [
    ...
    { id: "intent_first_gen", name: "Intent-first generation", layer: "abstraction", stages: ["intent"], status: "experimental",
      ...
      desc: "WIRED, verified with real output (2026-07-08, corrected same day — the first verification pass had self.champs silently empty on the primary, altern/agree/sem reranks were inert without disclosure; re-verified with the bug fixed). `Service.generate_intent_first()`: generates the punctuation sequence for the WHOLE response first (Punctuation sequence, above), then grows each word-span BACKWARD from its mark toward the previous mark using the Backward pair, with the mark's TYPE biasing what grows toward it (Exclaim affinity for '!', sent_type's question-affinity for '?'). Real verified sample: \"...The in little made in who? Which looks in different the film. Despite and made known won whom? ...Yourself go not may who!\" — real questions/exclamations tied to genuine discourse marks. Optional `crystallize` flag adds a forward polish pass (re-picks each word against its LEFT neighbor too, using the shipped forward Selection genome) — measured MIXED, not a clean win (func-func-adjacency -0.026 better, dangling-rate +0.068 worse, no visible readability gain) — stays OFF by default. HONEST VERDICT: does not fix fluency (still word salad, same clause-completeness gap as the forward pipeline) — this changes WHAT anchors generation, not the underlying word-to-word mechanics. `/api/evolang/intent_first`, \"Intent-first (backward)\" button on /evolang." },
    ...
    { id: "order_bigram", name: "Order-bigram", layer: "structural", stages: ["skeleton"], status: "experimental",
      short: "same question, K=1 only (diagnostic)",
      desc: "TRAINED, NOT wired into generation (2026-07-08). Part of the structural decomposition (full observability directive) — the same next-class question as Order, but with K=1 (tight local transition only) instead of K=4. val_ppl 11.344, beats the class-unigram baseline (12.984) — a real signal even at this narrow window. Left unwired because its trained artifact is a full class-LM (embedding + hidden layer), not a bilinear rerank matrix — doesn't fit the reranks-tuple shape every other genome here uses. Kept as a standalone DIAGNOSTIC scorer: score an actual generated class sequence under Order-bigram alone vs the full Order genome, and the gap tells you how much that sentence's structure depended on context beyond immediate class-bigram history." },
    ...
    { id: "sel_decomp", name: "Selection (decomposed)", layer: "structural", stages: ["fill"], status: "shipped",
      short: "backward-fit + forward-fit + frequency, split",
      desc: "WIRED, verified (2026-07-08). No retraining — Bidirectional's trained champion already stored three summed terms as separate parameters (ML=fit-vs-previous-word, MR=fit-vs-upcoming-class, beta=global-word-commonality-bias). Exposed as three independently-weighted gammas (SEL_BACKWARD_GAMMA/SEL_FORWARD_GAMMA/SEL_FREQ_GAMMA, all default 1.0) instead of one fixed ratio. Verified byte-identical output to the original at default weights, both locally and on the I2 primary. Full observability, not a behavior change." },
    ...
    { id: "altern_rhythm", name: "Altern-rhythm", layer: "structural", stages: ["skeleton", "fill"], status: "shipped",
      group: "altern_decomp", groupLabel: "Alternation (decomposed)",
      short: "coarse content/function rhythm only",
      desc: "WIRED, trained on the I2 primary (2026-07-08). The COARSE half of Alternation — sees only [is_content, bias] (2 features, deliberately blind to function subtype), trained on ALL adjacent pairs. val_acc 0.526, barely above chance: the coarse signal alone is weak, most of Alternation's real discriminative power lives in the subtype detail (see Altern-func-chain). Gated on the same `altern` toggle as its parent — transparent decomposition, not a new control. ALTERN_RHYTHM_GAMMA=1.0." },
    { id: "altern_funcchain", name: "Altern-func-chain", layer: "structural", stages: ["skeleton", "fill"], status: "shipped",
      group: "altern_decomp", groupLabel: "Alternation (decomposed)",
      short: "which specific func→func chains are legal",
      desc: "WIRED, trained on the I2 primary (2026-07-08). The FINE half of Alternation — full 14-feature subtype space, but trained ONLY on function-function adjacent pairs (of→the legal, the→of not, and→and never). val_acc 0.668, much stronger than the coarse split. Gated on the same `altern` toggle. ALTERN_FUNCCHAIN_GAMMA=1.5." },
    ...
    { id: "agree_modal", name: "Agree-modal", layer: "structural", stages: ["skeleton", "fill"], status: "shipped",
      group: "agree_decomp", groupLabel: "Agreement (decomposed)",
      short: "modal → bare verb form only",
      desc: "WIRED, trained on the I2 primary (2026-07-08). Mined ONLY modal-relevant pairs (\"could be\", not \"could is\") from Agreement's full 22-feature space. val_acc 0.768. Gated on the same `agree` toggle as its parent. AGREE_MODAL_GAMMA=1.5." },
    { id: "agree_number", name: "Agree-number", layer: "structural", stages: ["skeleton", "fill"], status: "shipped",
      group: "agree_decomp", groupLabel: "Agreement (decomposed)",
      short: "subject number → copula form only",
      desc: "WIRED, trained on the I2 primary (2026-07-08). Mined ONLY pronoun/finiteness-relevant pairs (\"he is\", \"they are\", \"I am\") from Agreement's full 22-feature space. val_acc 0.669. Gated on the same `agree` toggle. AGREE_NUMBER_GAMMA=1.5." },
    ...
    { id: "sem_adjacent", name: "Sem-adjacent", layer: "semantic", stages: ["fill"], status: "shipped",
      group: "sem_decomp", groupLabel: "Semantic (decomposed)",
      short: "distance-1 (tight collocation-like) only",
      desc: "WIRED, trained locally (2026-07-08). The tight half of Semantic — mined ONLY distance-1 (immediate-neighbor) content-word pairs, instead of Semantic's undifferentiated distance 1-4. val_acc 0.671. Gated on the same `sem` toggle as its parent. SEM_ADJACENT_GAMMA=1.5." },
    { id: "sem_window", name: "Sem-window", layer: "semantic", stages: ["fill"], status: "shipped",
      group: "sem_decomp", groupLabel: "Semantic (decomposed)",
      short: "distance-2..4 (loose topical) only",
      desc: "WIRED, trained locally (2026-07-08). The loose half of Semantic — mined ONLY distance-2..4 pairs. val_acc 0.539, much weaker than Sem-adjacent (0.671): loose topical fit is a genuinely harder signal than tight collocation. Gated on the same `sem` toggle. SEM_WINDOW_GAMMA=1.0." },
    ...
    { id: "clause_oblig", name: "Clause-obligation", layer: "abstraction", stages: ["boundary"], status: "cut",
      short: "track unclosed relative/subordinate clauses",
      desc: "CUT (2026-07-08) — same shape as Open-obligation, one level up: a relative pronoun/subordinator (who/which/that/whom/whose/where/when) pushes a depth counter, a verb-like word pops it, sentence-boundary probability suppressed while depth > 0. Direct attempt at the diagnosed clause-completeness gap (relative clauses that open and never resolve). Swept CLAUSE_GAMMA 0/1.5/3/5/8: never-closed-relative-rate dropped only 13.1%→12.5% and plateaus immediately past 1.5 (more pressure doesn't help further), while dangling-rate got worse (48.5%→53.3%). Samples showed no visible readability gain — some sentences ran very long, suggesting flat suppression holds boundary probability down too aggressively once triggered. A correctly-targeted idea whose blunt implementation costs more than it fixes — a sharper version (decay suppression over word-distance since the obligation opened, instead of a flat gamma) is a real follow-up, not attempted." },
