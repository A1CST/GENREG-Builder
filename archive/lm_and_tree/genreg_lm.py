...
class LMPopulation:
    def __init__(self, cfg, device):
        ...
        # -- low-rank trigram interaction channel (§VI, cfg["trigram"]) ------
        # logits += a_lr * ( bigram[c_t] + (E1[c_t] ⊙ E2[c_prev]) @ O ).
        # MULTIPLICATIVE — captures the char-pair interaction the additive
        # prev-char concat cannot express. Gated (a_lr init 0): transparent at
        # gen 0 so it bootstraps two-phase from a trained substrate.
        self.trigram = bool(cfg.get("trigram"))
        if self.trigram:
            Lr = cfg.get("lr_dim", 16)
            self.Lr = Lr
            self.E1 = mk(P, Vv, Lr, scale=0.1)
            self.E2 = mk(P, Vv, Lr, scale=0.1)
            self.O_lr = mk(P, Lr, Vv, scale=1.0 / np.sqrt(Lr))
            self.bigram_lr = torch.zeros(P, Vv, Vv, device=device)
            self.alpha_lr = torch.zeros(P, device=device)
        self.P, self.D, self.H = P, D, H

    def _trigram(self, cur, prev):
        """Multiplicative char-pair channel. cur/prev: (B,) or (P,B). -> (P,B,V)."""
        t = self.t
        if not self.trigram:
            return None
        if cur.dim() == 1:                        # shared context (evaluate)
            e1 = self.E1[:, cur, :]               # (P,B,Lr)
            e2 = self.E2[:, prev, :]
            big = self.bigram_lr[:, cur, :]       # (P,B,V)
        else:                                     # per-genome context (rollout)
            pi = t.arange(self.P, device=self.dev)[:, None]
            e1 = self.E1[pi, cur, :]
            e2 = self.E2[pi, prev, :]
            big = self.bigram_lr[pi, cur, :]
        inter = t.einsum("pbl,plv->pbv", e1 * e2, self.O_lr)
        return self.alpha_lr[:, None, None] * (big + inter)
    ...
    def _genome_tensors(self):
        ...
        if self.trigram:
            names += ["E1", "E2", "O_lr", "bigram_lr", "alpha_lr"]
        return names
    ...
    def _mutate(self, idx, rng_t):
        ...
        if self.attention:
            weighted += [("Wq_a", 1.0), ("Wk_a", 1.0), ("B_rel", 0.5),
                         ("alpha", 0.3)]
        if self.trigram:
            weighted += [("E1", 1.0), ("E2", 1.0), ("O_lr", 1.0),
                         ("bigram_lr", 1.0), ("alpha_lr", 0.3)]
        ...
        if getattr(self, "trigram_only", False):
            # two-phase: substrate frozen, only the trigram channel evolves
            keep = {"E1", "E2", "O_lr", "bigram_lr", "alpha_lr"}
            weighted = [(n, s) for n, s in weighted if n in keep]
        ...
        # activation ids: rare reassignment (frozen in composed / two-phase)
        if not getattr(self, "frozen_encoder", False) and not getattr(self, "trigram_only", False):
...
DEFAULTS = dict(pop=400, embed=24, hidden=64, seq_len=64, batch=96,
                ...
                trigram=False, trigram_only=False, lr_dim=16)
...
def run(cfg=None, log=print, resume=None):
    ...
    if cfg.get("trigram_only"):     # two-phase: freeze substrate, evolve channel
        pop.trigram_only = True
            ...
            if i >= n_prompt - 1:                      # start emitting
                if pop.tie:
                    ...
                    logits = (t.einsum("pbh,phv->pbv", h, pop.W_out[idx:idx + 1])
                              + pop.b_out[idx:idx + 1, None, :])[0, 0]
                if getattr(pop, "trigram", False):     # multiplicative channel
                    inter = (pop.E1[idx, cur] * pop.E2[idx, prev]) @ pop.O_lr[idx]
                    logits = logits + pop.alpha_lr[idx] * (pop.bigram_lr[idx, cur] + inter)
                if unk is not None:
