"""CIFAR v8: occlusion-contrastive diversity bank (user directive) — fitness =
novelty x entropy x occlusion-invariance-SNR, zero labels. 768 genomes, then
pure features + ceiling probe."""
import os
import sys
...
if __name__ == "__main__":
    ...
    print(f"v8 environment (occlusion-contrastive): {D['nf']} dims", flush=True)
    from sklearn.linear_model import LogisticRegression
    for C in (0.001, 0.003):
        clf = LogisticRegression(max_iter=3000, C=C)
        clf.fit(D["Ftr"], D["ytr"])
        print(f"CEILING v8 (C={C}): val {clf.score(D['Fva'], D['yva']):.4f} "
              f"test {clf.score(D['Fte'], D['yte']):.4f} "
              f"(jitter-stability 0.589; v5 label-driven 0.670)", flush=True)
