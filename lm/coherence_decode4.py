"""coherence_decode4.py - fluency round 4: topic-admitting guided pool +
sharpness-scaled steering.
...
def main():
    ...
    def gen(prompt, seed, temp, topk):
        ...
        for _ in range(24):
            ...
            if t_star is not None:
                ...
                pool = pool + topical
            top = np.array(pool[-topk - 2:] if pool else order[-3:])
            if len(top) == 1:
                k = int(top[0])
            else:
                z = lg[top] - lg[top].max()
    ...
    for arm in ARMS:
        ...
        for mode in ("single", "best8"):
            ...
            res["arms"].append(row)
            log(f"[coh4] {row['tag']}: judge {row['judge_logprob']} "
                f"d2 {row['distinct2']} hold {row['hold']}")

    res["seconds"] = round(time.time() - t0)
    with open(os.path.join(RD, "coherence_decode4_result.json"), "w") as f:
        json.dump(res, f, indent=1)
    log("COH4 RESULT: " + json.dumps([{k: v for k, v in a.items()
                                       if k != "samples"}
                                      for a in res["arms"]]))
    print("COH4 DONE", flush=True)
