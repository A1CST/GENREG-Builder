...
def load_evidence():
    """Word counts over the topic-stream TRAIN articles (the topic model's
    actual experience; the judge uses the DISJOINT test articles, so this
    filter cannot grade itself). Cached to json for pod runs."""
    from collections import Counter
    cache = os.path.join(RD, "topic_evidence_counts.json")
    ...
    for arts in TOPICS.values():
        for title in arts[:6]:                # TRAIN articles only
            _, text = zetifile.page_text(title)
            if text:
                cnt.update(_clean(text).split())
...
def target_topic_scores(torch, dev, responses, hm, hs, Wm, targets,
    ...
    With `evidence`, a word keeps its steering score ONLY if the topic
    model has actually experienced it (>= EVIDENCE_MIN occurrences in the
    train articles) - the V=5000 tail's proper names (aubrey/kathryn/...)
    have extrapolated, unreliable topic scores and become attractors."""
    ...
    if evidence is not None:
        keep = [k for k, w in enumerate(targets)
                if evidence.get(w, 0) >= EVIDENCE_MIN]
        mask = torch.zeros(len(targets), 1, device=dev)
        mask[torch.tensor(keep, device=dev)] = 1.0
        S = S * mask
        print(f"[steer] evidence floor: {len(keep)}/{len(targets)} targets "
              f"keep steering (>= {EVIDENCE_MIN} occurrences in train "
...
def main(smoke=False):
    ...
    topics = tm["topics"]
    ev = None if "--noev" in sys.argv else load_evidence()
    ...
    log(f"[steer] topic model: {len(tm['genomes'])} detectors, "
        ...
        f"{' (evidence floor ON)' if ev is not None else ''}")
    ...
    res["evidence_floor"] = ev is not None
    name = ("topic_steer_result_ev.json" if ev is not None
            else "topic_steer_result.json")
