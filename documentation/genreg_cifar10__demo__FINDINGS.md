...
## 2. Evolved Perception (V3) — Minutes to Seconds

The single biggest architectural improvement came from adding an evolved encoder between the raw board signals and the neural controller.

The motivation was biological: an eagle's retina doesn't pass raw photon counts to its brain. It has evolved edge detectors, motion sensors, and contrast filters that pre-process visual data before any decision-making happens. Perception and action are co-evolved. The brain doesn't learn to act on whatever signal the retina provides — the retina evolves to provide signal that makes action learnable.

V1 GENREG had a flat MLP controller that had to simultaneously learn what to see AND what to do with 868 parameters. V3 separates these concerns: the encoder (736 params) handles perception, the controller (1,188 params) handles decision-making.

The encoder's activation function is not fixed — it is selected per-genome from a catalog of 8 diverse nonlinearities (tanh_scaled, gated_linear, soft_threshold, resonance, dual_path, abs_gate, quadratic_relu, identity_plus). Each genome evolves not just the encoder weights but which mathematical lens to use and how to tune it. Two genomes can look at the same board and see completely different representations.

**The result was dramatic.** V1 required approximately 53 minutes of training to first achieve a 512 tile. V3 achieved 512 in under 5 seconds. The explanation: random initialization of the encoder produces immediate perceptual diversity. Some encoders, by chance, project the board state into representations that make good moves obvious. Selection amplifies these. The population doesn't spend thousands of generations learning to see — it starts with diverse perceptual hypotheses and selects the ones that work.

After training, the population typically converges on 1-2 activation functions. On the 2048 task, quadratic_relu dominated (500/500 genomes by generation 250). Evolution discovered the mathematically appropriate activation for a precision task without being told what to use.
...
## 4. Architecture Is Not the Bottleneck
