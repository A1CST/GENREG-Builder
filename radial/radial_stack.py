...
def feature_r0(torch, tp, env, g, test=False, want_grid=False):
    ...
    H = W = int(round(M.shape[2] ** 0.5))    # resolution-aware per map: train and
    #                                          test slots may be DIFFERENT resolutions,
    #                                          so read the spatial size from THIS map
    #                                          (square) instead of the basis literal.
...
def feature_vec(torch, tp, prevF, g):
    ...
    if gt:
        ...
        st = gt.get("stats")
        if st is not None:
            # environment-provided constants (baked at freeze time):
            # the genome is a pure per-row function
            gz = (gv - st[0]) / (st[1] + 1e-9)
        else:
            # evolution-time fallback: population batch statistics
            gz = (gv - gv.mean()) / (gv.std() + 1e-9)
        z = z * torch.sigmoid(gt["k"] * gz)
    return z


def bake_gate_stats(torch, tp, genomes, bank):
    """Freeze each gated genome's normalization into its genes: gate-
    stream mean/sd over the TRAINING environment. Features are the
    environment - statistics live in it, computed once; genomes must be
    pure functions of (row, environment), never of the runtime batch."""
    for g in genomes:
        gt = g.get("gate")
        if gt and gt.get("stats") is None:
            gv = bank[:, gt["c"] % bank.shape[1]]
            for prim, a, b in gt["prog"]:
                gv = tp[_PRIMS[prim]](a * gv + b)
            gt["stats"] = [float(gv.mean()), float(gv.std())]
    return genomes
